...
def run_model(): # TODO: implement model once it exists
    ...
    label.configure(image=grey_image_ctk, text="")  # Configuring the label to display the grayscale image
...
label.grid(row=1, column=0, columnspan=3)  # Placing the label in the grid layout
...
button_load.grid(row=5, column=0, pady=3) # Left
button_exit.grid(row=5, column=1, pady=3) # Middle
button_run.grid(row=5, column=2, pady=3) # Right
